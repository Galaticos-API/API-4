...
Aplicar npm run migrate em backend. A migration 010_hierarchy_archive.sql substitui a antiga migration da branch 005_hierarchy_archive.sql e também funciona caso aquela já tenha sido aplicada. Ela permite arquivado nos estados de feature/PBI, adiciona datas, preserva registros e corrige os defaults de novas entidades para rascunho.
...
- CI configura ARCHIVE_TEST_DATABASE_URL para banco exclusivo com sufixo _test e executa ambos após migrations. Docker local indisponível nesta sessão; resultado remoto ainda pendente.
