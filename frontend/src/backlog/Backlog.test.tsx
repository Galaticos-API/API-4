...
const Projects = ({
  ...
  <ProjectsPage pathname={pathname} canCreate />
);
...
it("reconhece as rotas aninhadas do backlog e rejeita caminhos fora do padrão", () => {
  ...
  expect(
    parseBacklogRoute(
      "/projects/p1/epics/e1/features/new",
    ),
  ).toEqual({
    screen: "feature-new",
    projectId: "p1",
  ...
  expect(
    parseBacklogRoute(
      "/projects/p1/epics/e1/features/f1",
    ),
  ).toEqual({
  ...
  expect(
    parseBacklogRoute(
      "/projects/p1/epics/e1/features/f1/pbis/new",
    ),
  ).toEqual({
    screen: "pbi-new",
    projectId: "p1",
  ...
  expect(
    parseBacklogRoute(
      "/projects/p1/epics/e1/features/f1/pbis/b1",
    ),
  ).toEqual({
    screen: "pbi-detail",
    projectId: "p1",
  ...
  expect(
    parseBacklogRoute("/projects/p1"),
  ).toBeNull();

  expect(
    parseBacklogRoute("/projects"),
  ).toBeNull();
});
...
it("PBI-01.1.2 Cenário 1: cria épico e navega ao seu detalhe", async () => {
  ...
  fireEvent.change(
    ...
    {
      target: {
        value: "Especificar o backlog",
      },
    },
  );

  fireEvent.click(
    screen.getByText("Criar épico", {
      selector: "button",
    }),
  );
...
it("PBI-01.1.2 Cenário 3: mostra os campos faltantes quando a conclusão é recusada", async () => {
  const request = vi.fn(
    async (url: string, init?: RequestInit) => {
      ...
      if (
        url ===
          `/api/v1/epics/${epic.id}/complete` &&
        init?.method === "PATCH"
      ) {
        return response(
          {
            error:
              "Não é possível concluir o épico.",
      ...
      if (
        url ===
        `/api/v1/features?epico_id=${epic.id}&limit=100`
      ) {
        return response({
          items: [],
          total: 0,
  ...
  render(
    <Projects
      pathname={`/projects/project-1/epics/${epic.id}`}
    />,
  );
...
it("PBI-01.1.3 Cenário 2: exibe o épico de origem ao abrir a feature", async () => {
  const feature = {
    ...
    pbis_count: 0,
    criterios_count: 0,
    epico_titulo: "Especificar o backlog",
    projeto_id: "project-1",
  };
  ...
  render(
    <Projects
      pathname={`/projects/project-1/epics/${epic.id}/features/${feature.id}`}
    />,
  );

  expect(
    await screen.findByText(
      "Épico: Especificar o backlog",
    ),
  ).toBeTruthy();
...
it("PBI-01.1.4 Cenário 2: apresenta os três blocos da história como campos distintos", async () => {
  ...
  render(
    <Projects pathname="/projects/project-1/epics/epic-1/features/feature-1/pbis/new" />,
  );
...
it("registra se um PBI exige interface para aplicar a regra de protótipo", async () => {
  ...
  await waitFor(() =>
    expect(window.location.pathname).toBe(
      "/projects/project-1/epics/epic-1/features/feature-1/pbis/pbi-ui",
    ),
  );
