...
export function QualityPanel({ report, title = "Painel de Qualidade", defaultExpanded = true }: QualityPanelProps) {
    ...
    <section className="glass-panel quality-panel" aria-label={title}>
      <header className="quality-panel-header">
        <div className="quality-panel-header-left">
          <span className="projects-eyebrow">Conformidade e maturidade</span>
        ...
        <div className="quality-panel-header-right">
          ...
          {report.score_completude !== null && (
            <span className={`badge ${getBadgeClass(report.score_completude)}`}>
          ...
          >
            {expanded ? "Recolher painel" : "Abrir painel de qualidade"}
          </button>
        </div>
      </header>

      {expanded && (
        <div id="quality-checklist-content" className="quality-panel-body">
          <div className="quality-rules-notice">
            <p className="quality-help-text">
              <strong>Bloqueio apenas na conclusão:</strong> você pode salvar rascunhos livremente a qualquer momento.
              As verificações marcadas com <em>[Bloqueia conclusão]</em> são exigidas exclusivamente para marcar o item como concluído.
            </p>
          </div>

          <ul className="quality-checklist" role="list">
            {report.checks.map((check: RealtimeQualityCheck) => {
              const isAlertOnly = !check.is_blocking && !check.passed;
              return (
                <li
                  key={check.check_id}
                  className={`quality-check-item ${check.passed ? "passed" : isAlertOnly ? "warning" : "failed"}`}
                  data-testid={`quality-check-${check.check_id}`}
                >
                  <div className="quality-check-indicator">
                    {check.passed ? (
                      <span className="quality-icon success" aria-label="Aprovado">✓</span>
                    ) : isAlertOnly ? (
                      <span className="quality-icon warning" aria-label="Alerta">⚠</span>
                    ) : (
                      <span className="quality-icon danger" aria-label="Reprovado">✗</span>
                    )}
                  </div>

                  <div className="quality-check-info">
                    <div className="quality-check-title-row">
                      <strong className="quality-check-name">{check.check_name}</strong>
                      {check.is_blocking ? (
                        <span className="quality-badge-tag blocking">Bloqueia conclusão</span>
                      ) : (
                        <span className="quality-badge-tag alert">Alerta informativo</span>
                      )}
                    </div>
                    <p className="quality-check-message">{check.message}</p>
                  </div>
                  ...
                  {!check.passed && (
                    <div className="quality-check-action">
                      <button
                        ...
                        onClick={() => scrollToField(check.target_field_id)}
