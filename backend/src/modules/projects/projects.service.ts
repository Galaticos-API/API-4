...
export class ProjectsService {
  ...
  async create(
    ...
    // Impedir nome duplicado entre projetos ativos.
    const existing =
      await this.repository.findActiveByName(
        dto.nome,
      );
  ...
  async getBacklogTree(
    ...
    const tree =
      await this.repository.findBacklogTree(
        id,
      );
  ...
  async archiveImpact(
    ...
    const impact =
      await this.repository.archiveImpact(id);
  ...
  async update(
    ...
    const parseResult =
      updateProjectSchema.safeParse(input);
