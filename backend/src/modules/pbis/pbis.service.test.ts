...
class InMemoryPbisRepository extends PbisRepository {
  ...
  async findById(id: string): Promise<PbiWithContext | null> {
    ...
    return {
      ...found,
      criterios_count: this.criteriosPorPbi.get(id) ?? 0,
      projeto_status: this.statusProjeto,
    };
  }
  ...
  async findAll(query: PbiQueryDTO): Promise<PaginatedPbis> {
    const items = this.pbis.filter(
      (p) => !query.feature_id || p.feature_id === query.feature_id,
    );
...
class StubCriteriaRepository extends CriteriaRepository {
  ...
  async listByEntity(
    _tipo: CriterionEntityType,
    id: string,
  ): Promise<Criterion[]> {
    return this.cenariosPorPbi.get(id) ?? [];
  }

  async listByEntities(
    _tipo: CriterionEntityType,
    ids: string[],
  ): Promise<Criterion[]> {
...
function setup() {
  ...
  featuresRepo.features.push({
    id: FEATURE_ID,
    epico_id: "c0000000-0000-4000-8000-000000000001",
    titulo: "Feature base",
    descricao: null,
    objetivo: null,
    prioridade: "Must",
...
test(
  "PBI-01.1.4 Cenário 1: cria PBI com história completa vinculado à feature com status rascunho",
  async () => {
    const { service } = setup();
...
test(
  "listagem recalcula completude quando os cenários mudam sem confiar no valor persistido",
  async () => {
    const {
      service,
      pbisRepo,
    ...
    const withScenario = await service.list({
      feature_id: FEATURE_ID,
    });

    assert.equal(
      withScenario.items[0].score_completude,
      100,
    );
    ...
    const withoutScenario = await service.list({
      feature_id: FEATURE_ID,
    });

    assert.equal(
      withoutScenario.items[0].score_completude,
      75,
    );
  },
);
...
test(
  "PBI-01.1.4 Cenário 3: impede conclusão sem nenhum cenário de aceitação",
  async () => {
    const { service } = setup();
...
test(
  "permite concluir PBI após registrar ao menos um cenário de aceitação estruturado",
  async () => {
    const {
      service,
      pbisRepo,
...
test(
  "impede cadastro de PBI em feature inexistente",
  async () => {
    const { service } = setup();
...
test(
  "PBI-01.1.4 Cenário 2: exige os três blocos da história separadamente",
  async () => {
    const { service } = setup();
...
test(
  "PBI-01.3.1 Cenário 3: impede conclusão de PBI com título fora do padrão de infinitivo",
  async () => {
    const {
      service,
      pbisRepo,
    ...
    await assert.rejects(
      ...
      (err: Error) => {
        ...
        assert.ok(
          details.campos_faltantes.includes(
            "titulo_infinitivo",
          ),
        );
...
test(
  "PBI-01.1.5 Cenário 3: impede edição e conclusão de PBI cujo projeto foi arquivado",
  async () => {
    const {
      service,
      pbisRepo,
...
test(
  "PBI-01.3.4 Cenário 2: permite concluir PBI com termos vagos sem bloqueio",
  async () => {
    const {
      service,
      pbisRepo,
    ...
    const created = await service.create({
      ...
      titulo: "Cadastrar item",
      historia_como_um: "Product Owner",
      historia_eu_quero: "um sistema rápido",
      historia_para_que: "fique bonito",
    });
...
test(
  "permite concluir PBI com título fora do infinitivo quando regra titulo_infinitivo está desabilitada",
  async () => {
    const pbisRepo = new InMemoryPbisRepository();
    const featuresRepo = new StubFeaturesRepository();
    const criteriaRepo = new StubCriteriaRepository();
...
test(
  "permite concluir PBI sem cenários quando cenario_estruturado está desabilitada",
  async () => {
    const pbisRepo = new InMemoryPbisRepository();
    const featuresRepo = new StubFeaturesRepository();
    const criteriaRepo = new StubCriteriaRepository();
    ...
    const created = await service.create({
      ...
      historia_como_um: "PO",
      historia_eu_quero: "cadastrar um item",
      historia_para_que: "organizar o backlog",
    });
...
test(
  "quality() retorna o relatório determinístico combinando título, história e cenários do PBI",
  async () => {
    const {
      service,
      criteriaRepo,
    ...
    criteriaRepo.cenariosPorPbi.set(created.id, [
      {
        ...
        dado: "d",
        quando: "q",
        entao: "o sistema deve ser rápido",
        ordem: 1,
        created_at: new Date().toISOString(),
      },
    ]);
    ...
    assert.equal(
      relatorio.titulo.aprovado,
      false,
    );

    assert.equal(
      relatorio.cenarios[0].aprovado,
      true,
    );

    assert.ok(
      relatorio.termos_vagos.some(
        (ocorrencia) =>
          ocorrencia.termos.includes("rápido"),
      ),
    );
  },
);
...
test(
  "impede cadastrar PBI em feature de projeto arquivado",
  async () => {
    const {
      service,
      featuresRepo,
    ...
    const FEATURE_PROJETO_ARQUIVADO =
      "b0000000-0000-4000-8000-000000000099";

    featuresRepo.features.push({
      id: FEATURE_PROJETO_ARQUIVADO,
      epico_id:
        "c0000000-0000-4000-8000-000000000001",
      titulo: "Feature órfã",
      descricao: "d",
